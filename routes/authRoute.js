...
router.post("/signup", async (req, res) => {
  try {
    ...
    const { firstname, lastname, password, email, phoneNumber, address } =
      req.body;
    ...
    const userData = {
      ...
      address: address,
      userID: uid(16),
      role: req.body.isDelivery ? "delivery" : "client",
    };
...
router.post("/change-password/:code", async (req, res) => {
  ...
  try {
    const user = await User.findOne({ resetPasswordCode: code });
    if (!user)
      return res.status(404).send({ error: "Token Expired or invalid." });
