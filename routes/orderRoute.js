...
router.post("/", userChecker, async (req, res) => {
  try {
    ...
    const itemsWithPrices = items.map((item) => {
      ...
      const quantity = Math.min(item.quantity, product.countInStock);

      if (quantity < item.quantity) {
        throw new Error(`${product.name} is insufficient.`);
      }
    ...
    const updatedProductCounts = items.map(async (item) => {
      const product = await Product.findOne({ productID: item.productID });
      product.countInStock = product.countInStock - item.quantity;
      return {
        updateOne: {
          filter: { productID: item.productID },
          update: { countInStock: product.countInStock },
        },
      };
    });

    await Product.bulkWrite(await Promise.all(updatedProductCounts));

    res.send(order);
  } catch (error) {
    console.error(error);
    res.status(400).send({ error: "Could not order delivery" });
  }
});
